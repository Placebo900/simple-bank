...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		name          string
		accountID     int64
		buildSteps    func(store *mockdb.MockStore)
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				...
				reqireBodyMathcAccount(t, recorder.Body, account)
			},
		},
		{
			name:      "NotFound",
...
func TestCreateAccountAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Create account with status OK",
			body: gin.H{
				"owner":    account.Owner,
		...
		{
			name: "Bad request while create account",
			body: gin.H{},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
		...
		{
			name: "Internal server error while create account",
			body: gin.H{
				"owner":    account.Owner,
				"currency": account.Currency,
			},
...
func reqireBodyMathcAccount(t *testing.T, body *bytes.Buffer, account db.Account) {
